<template>
    <v-card class="customFontSize">
        <v-card-title>
            <span class="headline">Edit Deduction</span>
        </v-card-title>
        ...
        methods: {
            ...
            async saveData() {
                ...
                if (this.$v.$invalid) {
                    ...
                    try {
                        const {data} = await axios.post('Deductions/UpdateDeductions', this.formData)
                        this.snackbar.message = data.success ? 'Deduction Updated Successfully' : data.errors.join(', ')

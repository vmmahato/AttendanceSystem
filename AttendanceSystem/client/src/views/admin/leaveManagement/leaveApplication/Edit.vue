<template>
    <v-card class="customFontSize">
        <v-card-title>
            <span class="headline">Edit Leave Application</span>
        </v-card-title>
        ...
        <v-card-actions>
            ...
            <v-btn color="teal darken-1" text @click="saveData" :disabled="haltOnNoLeaveRemaining" v-if="isPending">Save</v-btn>
        </v-card-actions>
    </v-card>
</template>
        ...
        methods: {
            ...
            getReplacementListAndOTData() {
                ...
                if (this.isReplacementLeave && employeeID && fromDate && toDate) {
                    axios.post('LeaveApplication/GetReplacementEmployeeList',
                        {
                        ...
                        axios.post('LeaveApplication/GetEmplyeeOT', {
                            employeeID
                        }).then(({data}) => {
                    ...
                    axios.post('LeaveApplication/GetEmplyeeRemainingLeave', {
                        leaveID,
                        employeeID
                    }).then(({data}) => {
                        this.formData.remainingLeave = data.remainingLeave
            ...
            async saveData() {
                ...
                if (this.$v.$invalid) {
                    ...
                    try {
                        ...
                        const {data} = await axios.post('LeaveApplication/UpdateLeaveApplication', this.formData)
                        this.snackbar.message = data.success ? 'Leave Application Updated Successfully' : data.errors.join(', ')

<template>
    <v-container>
        ...
        >
            ...
            <template #item.isReplacementLeave="{value}">
                <v-icon color="success" v-if="value"> mdi-check-bold</v-icon>
                <v-icon v-else color="error"> mdi-close-thick</v-icon>
            </template>
            <template #item.isPaidLeave="{value}">
                <v-icon color="success" v-if="value"> mdi-check-bold</v-icon>
                <v-icon v-else color="error"> mdi-close-thick</v-icon>
            </template>
            <template #item.isLeaveCarryable="{value}">
                <v-icon color="success" v-if="value"> mdi-check-bold</v-icon>
                <v-icon v-else color="error"> mdi-close-thick</v-icon>
            </template>
        ...
        <v-dialog max-width="700px"
            ...
            <TheDetailGrid v-if="snackbar.detailComponent"
    ...
    export default {
        ...
        methods: {
            editItem({leaveID}) {
                ...
                this.snackbar.getItemUrl = 'LeaveSetup/GetLeaveSetupByID/' + leaveID
            },
            showItem({leaveID}) {
                this.snackbar.dialog = true
            ...
            deleteItem({leaveID, leaveName}) {
                this.snackbar.dialog = true
                this.snackbar.deleteForm = true
                this.snackbar.deleteUrl = 'LeaveSetup/DeleteLeaveSetup/' + leaveID
                this.snackbar.deleteItemTitle = 'Leave ' + '   ' + leaveName
            },
            createItem() {
                this.snackbar.dialog = true
            ...
            async loadItems() {
                ...
                try {
                    const {data} = await axios.post("LeaveSetup/LeaveSetupList", param)
                    this.rows = data.data;

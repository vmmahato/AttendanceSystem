...
<script>
    ...
    export default {
        ...
        methods: {
            editItem({settlementID}) {
                this.snackbar.dialog = true
                this.snackbar.editForm = true
                this.snackbar.getItemUrl = 'LeaveSettlement/GetLeaveSettlementByID/' + settlementID
            },
            deleteItem({settlementID, employeeName}) {
                this.snackbar.dialog = true
                this.snackbar.deleteForm = true
                this.snackbar.deleteUrl = 'LeaveSettlement/DeleteLeaveSettlement/' + settlementID
                this.snackbar.deleteItemTitle = 'Settlement  ' + ' for  ' + employeeName
            },
            createItem() {
                this.snackbar.dialog = true
            ...
            async loadItems() {
                ...
                let param = {
                    ...
                    OrderBy: sortBy[0],
                    EmployeeName: this.filterParams.EmployeeName,
                }
                try {
                    const {data} = await axios.post("LeaveSettlement/LeaveSettlementList", param)
                    this.rows = data.data;

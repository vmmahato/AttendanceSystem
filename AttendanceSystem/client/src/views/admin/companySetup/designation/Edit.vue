<template>
    <v-card class="customFontSize">
        <v-card-title>
            <span class="headline">Edit Designation</span>
        </v-card-title>
        ...
        methods: {
            ...
            async saveData() {
                ...
                if (this.$v.$invalid) {
                    ...
                    try {
                        const {data} = await axios.post('Designation/UpdateDesignation', this.formData)
                        this.snackbar.message = data.success ? 'Designation Updated Successfully' : data.errors.join(', ')
